...
# 6. 64 channels global model trained,
#    64 channels final epoch SS,
#    *then select N channels based on SS EEGNet weights,
#    *train last epochs N channels SS by setting unused channels in 64-ch global model to zero.
...
results_dir=f'SS-TL_v6/{cs_model.NO_selected_channels}ch'
        ...
        for sub_idx in test_global:
            ...
            for train_sub, test_sub in kf_subject.split(X_sub, y_sub):
                ...
                if cs_eeg:
                    ...
                    X_sub_cs = np.zeros((X_sub.shape))
                    X_sub_cs[:,:,selected_channels,:] = X_sub[:,:,selected_channels,:]
                ...
                if not cs_eeg:
                    ...
                    model = models.EEGNet(nb_classes = cs_model.NO_classes, Chans=cs_model.NO_channels, Samples=480, regRate=0.25,
                    ...
                    weights_cs = np.zeros((model.layers[3].get_weights()[0].shape))
                    weights_cs[selected_channels,:,:,:] = model_global.layers[3].get_weights()[0][selected_channels,:,:,:]

                    model.layers[1].set_weights(model_global.layers[1].get_weights())
                    model.layers[2].set_weights(model_global.layers[2].get_weights())
                    model.layers[3].set_weights([weights_cs])

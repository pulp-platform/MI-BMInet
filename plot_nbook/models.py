...
from keras.layers import Input, Flatten, InputSpec
...
def EEGNet(nb_classes, Chans=64, Samples=128, regRate=.25,
    ...
    block1       = Conv2D(F1, (1, kernLength), padding = 'same',
                                   ...
                                   use_bias = False, name="tempConv")(input1)
    block1       = BatchNormalization(axis = 1)(block1)
    block1       = DepthwiseConv2D((Chans, 1), use_bias = False,
                                   depth_multiplier = D,
                                   depthwise_constraint = max_norm(1.),
                                   name='spatialConv')(block1)
    block1       = BatchNormalization(axis = 1)(block1)
...
def cubeEEGNet(nb_classes, Chans=64, Samples=128, regRate=.25,
    ...
    if dropoutType.lower() == 'spatialdropout2d':
        ...
        dropoutType = Dropout
    elif dropoutType.lower() == 'timedropout2d':
        dropoutType = TimeDropout2D
    else:
...
class TimeDropout2D(Dropout):
    """
    Dropout layer, defined (N, C, H, W), drops C and H
    """
    def __init__(self, rate, data_format=None, **kwargs):
        """
        See t.nn.Dropout2d for parameters
        """
        super(TimeDropout2D, self).__init__(rate, **kwargs)
        ...
        #print(self.data_format) # channels_last

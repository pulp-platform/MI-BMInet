...
def read_data(subjects , runs, PATH, long=False):
    ...
    DATA EXPLANATION:
        ...
        >> the array runs contains an array of all the runs.
        >> a run is one run in the array runs.
        >> a trial is one of the 3s clips taken from a 2 minute run.
        >> samples is the time of one trial * signal frequency
    ...
    # set up 3-fold using intra-subject
    fold1 = [4,6]
    fold2 = [8,10]
    fold3 = [12,14]

    X_fold1 = np.empty((0,NO_channels,Window_Length))
    y_fold1 = np.empty(0)
    X_fold2 = np.empty((0,NO_channels,Window_Length))
    y_fold2 = np.empty(0)
    X_fold3 = np.empty((0,NO_channels,Window_Length))
    y_fold3 = np.empty(0)
    ...
    for subject in subjects:
        for run in runs:
            ...
            if run in fold1:
                ...
                y_fold1 = np.concatenate((y_fold1,labels_int))
            elif run in fold2:
                X_fold2 = np.concatenate((X_fold2,data_step))
                y_fold2 = np.concatenate((y_fold2,labels_int))
            elif run in fold3:
                X_fold3 = np.concatenate((X_fold3,data_step))
                y_fold3 = np.concatenate((y_fold3,labels_int))
            else:
                X_fold1 = np.concatenate((X_fold1,data_step[0:7]))
                y_fold1 = np.concatenate((y_fold1,labels_int[0:7]))
                X_fold2 = np.concatenate((X_fold2,data_step[7:14]))
                y_fold2 = np.concatenate((y_fold2,labels_int[7:14]))
                X_fold3 = np.concatenate((X_fold3,data_step[14:21]))
                y_fold3 = np.concatenate((y_fold3,labels_int[14:21]))

        # pdb.set_trace()

    # concatenate arrays in order to get the whole data in one input array
    X = np.concatenate((X,X_fold1))
    y = np.concatenate((y,y_fold1))
    X = np.concatenate((X,X_fold2))
    y = np.concatenate((y,y_fold2))
    X = np.concatenate((X,X_fold3))

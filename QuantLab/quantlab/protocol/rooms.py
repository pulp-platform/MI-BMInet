...
from quantlab.protocol.sam import SAM
...
def train_sam(logbook, net, device, loss_fn, opt, train_l):
    """Run one epoch of the training experiment."""
    logbook.meter.reset()
    bar = FillingSquaresBar('Training \t', max=len(train_l))
    controllers = indiv.Controller.getControllers(net)

    sam_optimizer = SAM(net.parameters(), opt, rho=logbook.config['treat']['optimizer']['sam_rho'])
    ...
    for i_batch, data in enumerate(train_l):
        ...
        loss_fn(net(inputs), gt_labels).backward()

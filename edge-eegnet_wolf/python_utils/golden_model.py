"""
This file contains the golden model for the quantized edgeEEGNet in integer representation.
"""
...
class GoldenModel:
    """
    Golden edgeEEGNet Model
    """
    def __init__(self, config_file, net_file, clip_balanced=True, reorder_bn=False):
        """
...
class Layer1(Layer):
    """
    Convolution(channels) + BN
    """
    def __init__(self, net, C, T, F1, F2, clip_balanced=True, **params):
        self.name = "Layer 1: Convolution in Space + Batch Norm"
        self.C = C
        self.T = T
        self.F1 = F1
        self.F2 = F2
        self.input_shape = ((C, T))
        self.output_shape = ((F2, 1, T))
        self.clip_balanced = clip_balanced
...
class Layer2(Layer):
    """
    Convolution(time) + BN + ReLU + Pool
    """
    def __init__(self, net, C, T, F1, F2, reorder_bn=True, clip_balanced=True, **params):
        self.name = "Layer 2: Convolution in Time + Batch Norm + ReLU + Pooling"
        self.C = C
        self.T = T
        self.F1 = F1
        self.F2 = F2
        self.input_shape = ((F2, 1, T))
        ...
        self.float_weights = np.reshape(net["conv2.weightFrozen"], (self.F2, 1, 1, 64))
        # self.float_weights = np.flip(self.float_weights, (-1))
        assert self.weights.shape == (self.F2, 1, 1, 64)
        self.weights = np.reshape(self.weights, (self.F2, 64))

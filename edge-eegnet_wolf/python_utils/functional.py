...
def depthwise_conv_space(x, w):
    ...
    - w: np.array(shape: [F2, CH])

    Returns: np.array(shape: [F2, 1, T])
    """
    ...
    y = np.zeros((w.shape[0], 1, x.shape[1]), dtype=x.dtype)

    for k in range(w.shape[0]):
        for t in range(x.shape[1]):
            y[k, 0, t] = np.convolve(x[:, t], w[k,:], mode="valid")
    return y
...
def conv_time(x, w):
    ...
    Parameters:
    - x: np.array(shape: [F2, 1, T])
    - w: np.array(shape: [F2, 64])

    Returns: np.array(shape: [F2, T'])
    """
    ...
    x = np.pad(x, ((0, 0), (0, 0), padding))

    for k in range(w.shape[0]):
        y[k, :] = np.convolve(x[k, 0, :], w[k, :], mode="valid")

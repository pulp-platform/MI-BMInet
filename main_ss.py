...
#
#################################################
def save_results(first_eval,tr_hist,num_classes,sub,split,n_ds,n_ch,T,fl):
    ...
    results_str = f'{results_dir}/stats/ss_class_{num_classes}_ds{n_ds}_nch{n_ch}_T{T}_fl{fl}_sub{sub_str}_split_{split}.csv'
    np.savetxt(results_str, np.transpose(results))
    return results[0:2,-1]

def freezelayer(model, n_level, verbose=False):

    num_layer = {0:0,1:3,2:8,3:13}

    for layer in model.layers[:num_layer[n_level]]:
...
T = 3
#n_ch = 64
n_ch_list = [64]#[4,6,10,14,18,20]#[2,3,5,7,9,11,64] # number of channels {8,19,27,38,64}
verbose = 0 # verbosity for data loader and keras: 0 minimum, 

# retraining parameters
n_epochs = 10
lr = 1e-3
n_level_list = [0,1,2,3]
            ...
            for _, test_sub_global in kf_global.split(subjects):
                for sub_idx in test_sub_global:
                    ...
                    for train_sub, test_sub in kf_subject.split(X_sub, y_sub):
                        ...
                        model = freezelayer(model, n_level)
                        ...
                        # save results
                        acc[sub_idx,sub_split_ctr]=save_results(first_eval,history,num_classes,subject,sub_split_ctr,n_ds,n_ch,T,n_level)

...
results_dir=f'global/mcu_cubeai'
    ...
    for n_ch in n_ch_vec:
        for T in T_vec:
            for n_ds in n_ds_vec:
                ...
                # np.savez(PATH+f'{num_classes}class_mm_intra',X_Train = X_Train, y_Train = y_Train)
                npzfile = np.load(PATH+f'{num_classes}class_mm_intra.npz')
                ...
                for train, test in kf.split(X_Train_real, y_Train):
                    ...
                    if np.min(X_Train_cs) < 0:
                        X_Train_cs = X_Train_cs - np.min(X_Train_cs)
                    X_Train_cs = X_Train_cs / np.max(X_Train_cs)
                    X_Train_cs = np.transpose(X_Train_cs, (0,2,3,1))

                    os.makedirs(f'data_nnom', exist_ok=True)
                    np.savez(f'data_nnom/{num_classes}class_mm_intra_train_split{split_ctr}',X_Train = X_Train_cs[train], y_Train = y_Train_cat[train])
                    np.savez(f'data_nnom/{num_classes}class_mm_intra_test_split{split_ctr}',X_Train = X_Train_cs[test], y_Train = y_Train_cat[test])
